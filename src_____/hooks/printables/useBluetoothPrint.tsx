...
import { CancelledBillsReportResponse, CollectionReport, GstStatement, GstSummary, ItemReport, ItemsData, RefundReportData, SaleReport, ShowBillData, StockReportResponse } from "../../models/api_types"
...
export const useBluetoothPrint = () => {
    ...
    async function printReceipt(addedProducts: ItemsData[], netTotal: number, totalDiscountAmount: number, cashAmount?: number, returnedAmt?: number, customerName?: string, customerPhone?: string, rcptNo?: number, paymentMode?: string) {
        ...
        let gstTotals = gstFilterationAndTotals(addedProducts, receiptSettings?.gst_type)
        ...
        try {
                ...
                if (item?.item_name?.length > 9) {
                    ...
                    receiptSettings?.discount_flag === "Y" && receiptSettings?.discount_position !== "B"
                        ...
                        : await BluetoothEscposPrinter.printColumn(
                            ...
                            //@ts-ignore
                            ["", item?.quantity.toString(), item?.price.toString(), calculateAmountAfterAmountDiscountPerProduct(item?.price, item?.quantity, 0)],
                            // : await BluetoothEscposPrinter.printColumn(
                            //     columnWidthsProductsHeaderAndBody,
                            //     [
                            //         BluetoothEscposPrinter.ALIGN.LEFT,
                    ...
                    receiptSettings?.discount_flag === "Y"
                        ...
                        : await BluetoothEscposPrinter.printColumn(
                            ...
                            //@ts-ignore
                            [item?.item_name, item?.quantity.toString(), item?.price.toString(), calculateAmountAfterAmountDiscountPerProduct(item?.price, item?.quantity, 0)],
                            {},
                        )
                    // : await BluetoothEscposPrinter.printColumn(
                    //     columnWidthsProductsHeaderAndBody,
            ...
            receiptSettings?.gst_type === "E"
                ...
                : await BluetoothEscposPrinter.printColumn(
                    ...
                    // ["TOTAL", ":", `${(netTotal - totalDiscountAmount + totalGST).toFixed(2)}`],
                    ["TOTAL", ":", `${netTotalWithGSTInclCalculate(netTotal, totalDiscountAmount)}`],
                    {},
                )
            // : await BluetoothEscposPrinter.printColumn(
            //     columnWidths,
            ...
            if (paymentMode === "D") {
                ...
                receiptSettings?.gst_type === "E"
                    ...
                    : await BluetoothEscposPrinter.printText(
                        `RECEIVED:       ${grandTotalWithGSTInclCalculate(netTotal, totalDiscountAmount)} [CARD]`,
                        { align: "center" },
                    )
                await BluetoothEscposPrinter.printText("\n", {})
                await BluetoothEscposPrinter.printText(
            ...
            if (paymentMode === "U") {
                receiptSettings?.gst_type === "E"
                    ...
                    : await BluetoothEscposPrinter.printText(
                        `RECEIVED:       ${grandTotalWithGSTInclCalculate(netTotal, totalDiscountAmount)} [UPI]`,
                        { align: "center" },
                    )
                await BluetoothEscposPrinter.printText("\n", {})
                await BluetoothEscposPrinter.printText(
    ...
    async function rePrintWithoutGst(addedProducts: ShowBillData[], netTotal: number, totalDiscountAmount: number, cashAmount?: number, returnedAmt?: number, customerName?: string, customerPhone?: string, rcptNo?: number, paymentMode?: string, isRefunded?: boolean) {
        const loginStore = JSON.parse(loginStorage.getString("login-data"))
        const fileStore = fileStorage.getString("file-data")
        ...
        try {
            ...
            addedProducts[0]?.discount_flag === "Y" && addedProducts[0]?.discount_position !== "B"
                ? await BluetoothEscposPrinter.printColumn(
                    columnWidthsProductsHeaderAndBody,
                    [
                        BluetoothEscposPrinter.ALIGN.LEFT,
                ...
                if (item?.item_name?.length > 9) {
                    ...
                    discountFlag === "Y" && discountPosition !== "B"
                        ...
                        : await BluetoothEscposPrinter.printColumn(
                            ...
                            ["", item?.qty.toString(), item?.price.toString(), calculateAmountAfterAmountDiscountPerProduct(item?.price, item?.qty, 0)],
                            {},
                        )
                    // : await BluetoothEscposPrinter.printColumn(
                    //     columnWidthsProductsHeaderAndBody,
                    ...
                    discountFlag === "Y" && discountPosition !== "B"
                        ...
                        : await BluetoothEscposPrinter.printColumn(
                            ...
                            //@ts-ignore
                            [item?.item_name, item?.qty.toString(), item?.price.toString(), calculateAmountAfterAmountDiscountPerProduct(item?.price, item?.qty, 0)],
                            {},
                        )
                    // : await BluetoothEscposPrinter.printColumn(
                    //     columnWidthsProductsHeaderAndBody,
            ...
            if (isRefunded)
                await BluetoothEscposPrinter.printText(
                    `REFUNDED AMT:    ${grandTotalCalculate(netTotal, totalDiscountAmount)}`,
                    { align: "center" },
                )
            await BluetoothEscposPrinter.printText("\n", {})
    ...
    async function rePrint(addedProducts: ShowBillData[], netTotal: number, totalDiscountAmount: number, cashAmount?: number, returnedAmt?: number, customerName?: string, customerPhone?: string, rcptNo?: number, paymentMode?: string, isRefunded?: boolean) {

        console.log("SSSSSSSSSSSSSSSS==================SSSSSSSSSSSSSSSSSS CALLED rePrint")
        ...
        let gstTotals = gstFilterationAndTotalForRePrint(addedProducts, addedProducts[0]?.gst_type)
        let { totalGST } = gstTotals // Destructure totalGST for separate handling
        ...
        try {
            ...
            if (isRefunded)
                addedProducts[0]?.gst_type === "E"
                    ?
                    await BluetoothEscposPrinter.printText(
                        `REFUNDED AMT:    ${grandTotalWithGSTCalculate(netTotal, totalDiscountAmount, totalGST)}`,
                        { align: "center" },
                    )
                    : await BluetoothEscposPrinter.printText(
                        `REFUNDED AMT:    ${grandTotalWithGSTInclCalculate(netTotal, totalDiscountAmount)}`,
                        { align: "center" },
                    )
            await BluetoothEscposPrinter.printText("\n", {})
    ...
    async function printRefundReport(refundReport: RefundReportData[], fromDate: string, toDate: string) {
        const loginStore = JSON.parse(loginStorage.getString("login-data"))
        const fileStore = fileStorage.getString("file-data")
            ...
            for (const item of refundReport) {
                ...
                receiptSettings?.gst_flag === "Y" ? (
                    await BluetoothEscposPrinter.printColumn(
                        ...
                        [item?.refund_rcpt_no?.toString()?.substring(item?.refund_rcpt_no?.toString()?.length - 4), item?.no_of_items?.toString(), item?.price?.toFixed(2)?.toString(), item?.discount_amt?.toString(), totalGST?.toString(), (item?.net_amt + item?.round_off)?.toString()],
                        {},
                    )
                ) : (
                    await BluetoothEscposPrinter.printColumn(
                        ...
                        [item?.refund_rcpt_no?.toString()?.substring(item?.refund_rcpt_no?.toString()?.length - 4), item?.no_of_items?.toString(), item?.price?.toFixed(2)?.toString(), item?.discount_amt?.toString(), "", (item?.net_amt + item?.round_off)?.toString()],
                        {},
                    )
                )
            }
