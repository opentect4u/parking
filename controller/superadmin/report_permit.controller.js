...
const getReportPermission = (cust_id) => {
...
const getAllReportList = (id = 0,cust_id) => {
    return new Promise(async (resolve, reject) => {
      var edit_report_data= await db_Select("*","md_admin_report_permit",id > 0 ? `customer_id = ${cust_id} AND sl_no = ${id}` : null,
        null
      );
...
const report_permit = async(req,res)=>{
 try{
    ...
    console.log(selected.cust_id,'cust');
    

    var cust = await getAllCustomerList();
    var report = await getReportPermission(selected.cust_id);
  ...
  const save_edit_report_permitt = async (req, res) => {
    try {
      ...
      let fields = value.id > 0 ? `detail_report='${value.detail_flag == "Y" ? "Y" : "N"}',veh_wise_report='${value.vehicle_flag == "Y" ? "Y" : "N"}',dev_wise_report='${value.device_flag == "Y" ? "Y" : "N"}',operate_wise_report='${value.operator_flag == "Y" ? "Y" : "N"}',shift_wise_report='${value.shift_flag == "Y" ? "Y" : "N"}',updated_by='${user_name}',updated_at='${datetime}'` : "(customer_id,detail_report,veh_wise_report,dev_wise_report,operate_wise_report,shift_wise_report,created_by,created_at)",
        values = `('${value.cust_id}','${value.detail_flag == "Y" ? "Y" : "N"}','${value.vehicle_flag == "Y" ? "Y" : "N"}','${value.device_flag == "Y" ? "Y" : "N"}','${value.operator_flag == "Y" ? "Y" : "N"}','${value.shift_flag == "Y" ? "Y" : "N"}','${user_name}','${datetime}')`;
...
module.exports = {report_permit, report_permit_edit,save_edit_report_permitt}