...
export default function OutpassScreen({ navigation }) {
  ...
  const handleUploadOutPassData = async (receipt_no, index) => {
    ...
    if (gstSettings && gstSettings[0]?.gst_flag == "Y") {
      gstPrice = await useGstPriceCalculator(gstSettings[0], price)
    }
            ...
            {serchData &&
              serchData.map((props, index) => {
                ...
                return (
                  <TouchableOpacity
                    key={index}
                    onPress={() => handleUploadOutPassData((props.receipt_no).toString(), index)}
                    style={{
