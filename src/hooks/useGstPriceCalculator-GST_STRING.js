...
function useGstPriceCalculator(gstSettings, parkingFees, gst_flag) {
    ...
    if (gstSettings.gst_type == "I") {
        price = (parkingFees * 100) / ((parseInt(Number(gstSettings.cgst)) + parseInt(Number(gstSettings.sgst))) + 100)
        price = Math.round(price * 100) / 100

        CGST = price * ((parseInt(Number(gstSettings.cgst))) / 100)
        CGST = Math.round(CGST * 100) / 100
        SGST = price * ((parseInt(Number(gstSettings.sgst))) / 100)
    ...
    if (gstSettings.gst_type != "I") {
        price = parkingFees
        CGST = parkingFees * ((parseInt(Number(gstSettings.cgst))) / 100)
        CGST = Math.round(CGST * 100) / 100
        SGST = parkingFees * ((parseInt(Number(gstSettings.sgst))) / 100)
        SGST = Math.round(CGST * 100) / 100
        console.log(CGST)
        console.log(SGST)
    ...
    price = parkingFees / (1 + (Number(gstSettings.cgst) + Number(gstSettings.sgst)) / 100)
    cgstAmount = sgstAmount = ((parkingFees - price)) / 2
